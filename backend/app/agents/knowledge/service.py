...
from app.agents.knowledge.repository import save_metric, save_document, save_chunks, update_document_status
from app.core.config import settings

logger = logging.getLogger("decision_os.knowledge.service")

TRANSCRIPT_DEPARTMENT = "transcripts"
TRANSCRIPT_OWNER = "system"
TRANSCRIPT_VERSION = "1.0"
TRANSCRIPT_TAGS = ["transcript", "auto-indexed"]


async def _index_transcript(workflow_id: str, transcript: str) -> None:
    """Auto-index the workflow transcript into the knowledge base as a searchable document."""
    from app.agents.knowledge.chunking import chunk_document_sections
    try:
        async with SessionLocal() as session:
            # Save document metadata
            doc_id = uuid.uuid4()
            storage_name = f"transcript_{workflow_id}.txt"
            content_bytes = transcript.encode("utf-8")
            db_doc = await save_document(
                session=session,
                name=f"Transcript: {workflow_id}",
                storage_name=storage_name,
                size_bytes=len(content_bytes),
                mime_type="text/plain",
                department=TRANSCRIPT_DEPARTMENT,
            ...
            sections = [{"content": transcript, "page": 1, "section": "Transcript"}]
            chunks = chunk_document_sections(sections, chunk_size=500, chunk_overlap=100)
            if not chunks:
                await update_document_status(session, db_doc.id, "failed", "No content to chunk")
                return
            ...
            await save_chunks(session, db_doc.id, chunks)
            await update_document_status(session, db_doc.id, "completed")
            logger.info(f"Auto-indexed transcript for workflow {workflow_id} — {len(chunks)} chunks")
    except Exception as e:
        logger.warning(f"Failed to auto-index transcript for workflow {workflow_id}: {e}")
...
class KnowledgeService(KnowledgeAgent):
    ...
    async def execute(self, state: WorkflowState) -> WorkflowState:
        ...
        # Auto-index the transcript so it's searchable in future queries
        if state.transcript and state.transcript.strip():
            await _index_transcript(state.workflow_id, state.transcript)
