...
logger = logging.getLogger("decision_os.reflection.providers.gemini")
...
class GeminiReflectionProvider(ReflectionProvider):
    def __init__(self, api_key: str, model: str) -> None:
        ...
        logger.info(f"Initialized Gemini Reflection Provider with model {model}")

    async def evaluate_explainability(self, *, system_prompt: str, user_prompt: str) -> dict:
        full_prompt = f"{system_prompt}\n\n{user_prompt}"
        logger.info("Calling Gemini API for strategy explainability...")
        try:
